...
export function formatDate(
  ...
  opts: { utc?: boolean; withTime?: boolean } = {}
): string {
  if (!iso) return "";
  const d = new Date(iso);
  if (Number.isNaN(d.getTime())) return "";

  const { utc = false, withTime = false } = opts;
 ...
 */
export function useBsToast(opts: {
  message: string;
  title?: string;
  /** @default: 2000 */
  delay?: number;
  ...
  const icon = opts.type === "success" ? { type: "fa-check-circle", color: "text-success" } :
    opts.type === "error" ? { type: "fa-times-circle", color: "text-danger" } :
      opts.type === "info" ? { type: "fa-info-circle", color: "text-info" } :
        null;
  toast.innerHTML = `
 ...
 */
export async function copyTextWithToast(text: string, opts?: {
  ...
  if (ok) {
    useBsToast({
      message: opts?.successMessage ?? "Copied to clipboard",
    ...
    useBsToast({
      message: opts?.failureMessage ?? "Copy failed",
      title: "Error",
