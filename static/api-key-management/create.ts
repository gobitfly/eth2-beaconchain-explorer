...
import { copyTextWithToast, validateApiKeyName, downloadJSON, safeFilename, renderKeyData, clearKeyData } from "./utils";
...
export function useAPIKeyCreation(api: ApiKeysClient, opts: Partial<typeof DEFAULTOPTIONS> & { onCreated?: () => void; } = {}) {
  const options = { ...DEFAULTOPTIONS, ...opts };

  const createForm = document.querySelector(options.createFormSelector) as HTMLFormElement | null;
  const keyNameinput = document.querySelector(options.keyNameinputSelector) as HTMLInputElement | null;

  if (!createForm || !keyNameinput) return;

  const validateNameAndSetStatus = useNameValidator(createForm, options.rulesSelector);
  ...
  // submit flow
  const submitBtn = createForm.querySelector<HTMLButtonElement>(options.submitBtnSelector);
  ...
  createForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      try { options.onCreated?.(); } catch (err) { console.error("onCreated callback failed:", err); }
    } catch (err) {
...
function useCreationModal(opts?: Partial<typeof DEFAULTCREATIONMODALOPTIONS>) {
  const options = { ...DEFAULTCREATIONMODALOPTIONS, ...opts };
  const modalContainer = document.querySelector(options.creationModalSelector) as HTMLElement | null;
  if (!modalContainer) return null;

  let createdKey: CreateAPIKeyData | null = null;
  const modal = useBSModal(modalContainer, {
    onHidden: () => {
  ...
  const loadingContainer = modalContainer.querySelector(options.loadingSectionSelector) as HTMLElement | null;
  const successContainer = modalContainer.querySelector(options.successSectionSelector) as HTMLElement | null;
  const errorContainer = modalContainer.querySelector(options.errorSectionSelector) as HTMLElement | null;
  ...
  function setAPIKeyInSuccessSection(key: CreateAPIKeyData) {
    ...
    const keyField = successContainer.querySelector(options.createdApiKeyRawSelector) as HTMLInputElement | null;
    if (keyField) {
  ...
  modalContainer.addEventListener("click", (event) => {
    ...
    const triggerDownload = target && target.closest("[data-download-api-key]") as HTMLElement | null;
    if (triggerDownload) {
      event.preventDefault();
      if (!createdKey) return;
      const payload = {
        provider: "beaconcha.in",
        exported_at: new Date().toISOString(),
        api: {
          base_url: 'https://beaconcha.in/api/v1/',
          docs_url: 'https://beaconcha.in/api/v1/docs',
        },
        ...createdKey,
      }
      const filename = safeFilename(`${createdKey.api_key?.name}-api-key`);
  ...
  return {
    showLoading() {
      modal.setOptiopns({ backdrop: "static", keyboard: false });
      modal.hideHeader();
      showSection("loading");
      modal.open();
    },
    showSuccess(key: CreateAPIKeyData) {
      modal.setOptiopns({ backdrop: "static", keyboard: false });
      modal.seTitle("Operation successful");
      setAPIKeyInSuccessSection(key);
      showSection("success");
      modal.open();
    },
    showError() {
      modal.setOptiopns({ backdrop: true, keyboard: true });
      modal.seTitle("Something went wrong.");
      showSection("error");
      modal.open();
    },
  }
