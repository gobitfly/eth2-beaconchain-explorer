var bannerContainer=document.querySelector(".info-banner-container"),bannerSearch=document.querySelector(".info-banner-search"),bannerSearchIcon=document.getElementById("banner-search"),bannerSearchInput=document.getElementById("banner-search-input");bannerSearch.addEventListener("click",function(){bannerContainer.classList.add("searching"),bannerSearchInput.focus()}),bannerSearchInput.addEventListener("blur",function(){bannerContainer.classList.remove("searching")});function updateBanner(){fetch("/latestState").then(function(res){return res.json()}).then(function(data){var epochHandle=document.getElementById("banner-epoch-data");data.currentEpoch&&(epochHandle.textContent=data.currentEpoch);var ethPriceHandle=document.getElementById("banner-eth-price-data");data.ethTruncPrice&&(ethPriceHandle.innerHTML=data.currencySymbol+" "+data.ethTruncPrice);var slotHandle=document.getElementById("banner-slot-data");data.currentSlot&&(slotHandle.textContent=data.currentSlot);var finDelayDataHandle=document.getElementById("banner-fin-data");if(finDelayHtml=`
