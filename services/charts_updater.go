...
var ChartHandlers = map[string]chartHandler{
	...
	"deposits_per_day":               {13, eth1DepositsPerDayChartData2},
	"deposits_distribution":          {13, depositsDistributionChartData2},
...
func eth1DepositsPerDayChartData2() (*types.GenericChartData, error) {
	var err error
	...
	/*
		select
			date_trunc('day', block_ts) as time,
			(sum(amount)/1e9)::int as amount,
		...
		with 
			deposits_per_day as (
				select
					...
					date_trunc('day', block_ts) as time,
					(sum(amount)/1e9)::int as amount,
					row_number() over (partition by date_trunc('day', block_ts) order by sum(amount) desc) as rank
				from eth1_deposits 
				where valid_signature = true
				group by from_address, time
				order by time, amount desc
			)
		select address, time, amount, rank from deposits_per_day where rank < $1
		union all
		(select 
			'' as address, 
			time, 
			...
			$1 as rank
		from deposits_per_day where rank >= $1 group by time)
		order by time, rank`, n)

	if err != nil {
		return nil, fmt.Errorf("error getting eth1-deposits-per-day: %w", err)
	}

	series := []*types.GenericChartDataSeries{}
	seriesByAddr := map[string]*types.GenericChartDataSeries{}
	for i := range rows {
		addr := string(utils.FormatEth1AddressString(rows[i].Address))
		if addr == "0x0000000000000000000000000000000000000000" {
			addr = "others"
		}
		d := []float64{
			float64(rows[i].Time.Unix() * 1000),
			float64(rows[i].Amount),
		}
		s, exists := seriesByAddr[addr]
		if !exists {
			ss := types.GenericChartDataSeries{
				Name: addr,
				Data: [][]float64{d},
			}
			series = append(series, &ss)
			seriesByAddr[addr] = &ss
		} else {
			s.Data = append(s.Data.([][]float64), d)
		}
	}

	chartData := &types.GenericChartData{
		Title:        "Daily deposited ETH",
		Subtitle:     "History of deposited ETH per day per Eth1-Address.",
		XAxisTitle:   "",
		YAxisTitle:   "ETH",
		StackingMode: "normal",
		Type:         "column",
...
func depositsDistributionChartData2() (*types.GenericChartData, error) {
	var err error
	...
	for i := range rows {
		if rows[i].Count*1000/totalDeposits < 2 { // addresses with <0.N% of deposits are "others"
			othersItem.Y += rows[i].Count
			continue
		}
		// if i > 100 {
	...
	subtitle := fmt.Sprintf("There are %v deposits (%v ETH, %.2f%% of needed genesis-deposits) from %v Eth1-Addresses. Only highest depositer per publickey is and only deposits that result in an active genesis-validator are counted.", totalDeposits, totalDeposits*32, genesisPercent, totalEth1Addresses)
...
func depositsDistributionChartData3() (*types.GenericChartData, error) {
	var err error
