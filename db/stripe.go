...
func StripeGetUserAPISubscription(id uint64) (types.UserSubscription, error) {
	...
	err := FrontendDB.Get(&userSub, "SELECT id, email, stripe_customerid, subscription_id, price_id, active, api_key FROM users LEFT JOIN users_stripe_subscriptions ON users.stripe_customerid = users_stripe_subscriptions.customer_id WHERE users.id = $1 AND price_id IN ($2) ORDER BY active desc LIMIT 1", id, priceIds)
	return userSub, err
}
