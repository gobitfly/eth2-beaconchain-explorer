...
func GetUserPushTokenByIds(ids []uint64) (map[uint64][]string, error) {
	...
	err := FrontendDB.Select(&rows, "SELECT user_id, notification_token FROM users_devices WHERE user_id = ANY($1) AND notify_enabled = true AND active = true AND notification_token IS NOT NULL ORDER BY id DESC GROUP BY user_id, notification_token ", pq.Array(ids))
	if err != nil {
		return nil, err
	}
	for _, r := range rows {
