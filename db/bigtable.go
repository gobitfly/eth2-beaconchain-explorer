...
func (bigtable *Bigtable) getValidatorSyncDutiesHistoryClickhouse(validators []uint64, startSlot uint64, endSlot uint64) (map[uint64]map[uint64]*types.ValidatorSyncParticipation, error) {
	...
	query := `
		SELECT
			validator_sync_committee_votes_epoch.validator_index,
			validator_sync_committee_votes_epoch.epoch,
			validator_sync_committee_votes_epoch.slot,
			validator_sync_committee_votes_epoch.executed
		FROM validator_sync_committee_votes_epoch FINAL
		WHERE
			validator_sync_committee_votes_epoch.epoch_timestamp >= ? AND
			validator_sync_committee_votes_epoch.epoch_timestamp <= ? AND
			validator_sync_committee_votes_epoch.validator_index IN (?) AND
			validator_sync_committee_votes_epoch.slot >= ? AND
			validator_sync_committee_votes_epoch.slot <= ?
		ORDER BY epoch ASC, slot ASC`
...
func (bigtable *Bigtable) getValidatorIncomeDetailsHistoryClickhouse(validators []uint64, startEpoch uint64, endEpoch uint64) (map[uint64]map[uint64]*itypes.ValidatorEpochIncome, error) {
	...
	for _, r := range rows {
		...
		if r.SyncRewards > 0 {
			...
			income.SyncCommitteePenalty = uint64(r.SyncRewards * -1)
		}
