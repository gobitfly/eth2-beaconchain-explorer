	...
	for len(tableData) < minimumTransactionsPerUpdate && pageTokenId != 0 {
		...
		for _, v := range t {
			...
			tableData = append(tableData, []interface{}{
				...
				toText,
				utils.FormatAmountFormatted(new(big.Int).SetBytes(v.GetValue()), "Ether", 8, 4, true, false),
				utils.FormatAmountFormatted(db.CalculateTxFeeFromTransaction(v, new(big.Int).SetBytes(b.GetBaseFee())), "Ether", 8, 4, true, false),
			})
		}
