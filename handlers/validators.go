...
func parseValidatorsDataQueryParams(r *http.Request) (*ValidatorsDataQueryParams, error) {
	...
	if err != nil {
		logger.WithError(err).WithField("route", r.URL.String()).Warn("parse validators data query params: error converting draw parameter from string to int")
		return nil, err
	}

	start, err := strconv.ParseUint(q.Get("start"), 10, 64)
	if err != nil {
		logger.WithError(err).WithField("route", r.URL.String()).Warn("parse validators data query params: error converting start parameter from string to int")
		return nil, err
	}
	if start > 10000 {
	...
	if err != nil {
		logger.WithError(err).WithField("route", r.URL.String()).Warn("parse validators data query params: error converting length parameter from string to int")
		return nil, err
	}
	if length < 0 {
		length = 100
...
func ValidatorsData(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		// the only way this happens is if strconv.ParseUint fails, which is almost certainly a client error
