...
func ValidatorsSlashingsData(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.WithError(err).WithField("route", r.URL.String()).Warn("validators slashings data: error converting draw parameter from string to int")
	...
	if err != nil {
		logger.WithError(err).WithField("route", r.URL.String()).Warn("validators slashings data: error converting start parameter from string to int")
		http.Error(w, "Error: Invalid start parameter", http.StatusBadRequest)
		return
	}
	length, err := strconv.ParseUint(q.Get("length"), 10, 64)
	if err != nil {
		logger.WithError(err).WithField("route", r.URL.String()).Warn("validators slashings data: error converting length parameter from string to int")
