...
func WithdrawalsData(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.WithError(err).WithField("route", r.URL.String()).Warn("withdrawals data: error converting draw parameter from string to int")
	...
	if err != nil {
		logger.WithError(err).WithField("route", r.URL.String()).Warn("withdrawals data: error converting start parameter from string to int")
	...
	if err != nil {
		logger.WithError(err).WithField("route", r.URL.String()).Warn("withdrawals data: error converting length parameter from string to int")
...
func BLSChangeData(w http.ResponseWriter, r *http.Request) {
	...
	draw, err := strconv.ParseUint(q.Get("draw"), 10, 64)
	if err != nil {
		logger.WithError(err).WithField("route", r.URL.String()).Warn("bls changes data: error converting draw parameter from string to int")
		http.Error(w, "Error: Invalid draw parameter", http.StatusBadRequest)
		return
	}
	start, err := strconv.ParseUint(q.Get("start"), 10, 64)
	if err != nil {
		logger.WithError(err).WithField("route", r.URL.String()).Warn("bls changes data: error converting start parameter from string to int")
		http.Error(w, "Error: Invalid start parameter", http.StatusBadRequest)
		return
	}
	if start > 10000 {
	...
	length, err := strconv.ParseUint(q.Get("length"), 10, 64)
	if err != nil {
		logger.WithError(err).WithField("route", r.URL.String()).Warn("bls changes data: error converting length parameter from string to int")
		http.Error(w, "Error: Invalid length parameter", http.StatusBadRequest)
		return
	}
	if length > 100 {
