...
func PoolsRocketpoolDataDAOProposals(w http.ResponseWriter, r *http.Request) {
	...
	for _, row := range dbResult {
		...
		if len(row.Payload) > 4 {
			entry = append(entry, fmt.Sprintf(`<span>%x…%x<span><i class="fa fa-copy fa-lg text-muted ml-2 p-1" role="button" data-toggle="tooltip" title="Copy to clipboard" data-clipboard-text="%x"></i>`, row.Payload[:2], row.Payload[len(row.Payload)-2:], row.Payload))
