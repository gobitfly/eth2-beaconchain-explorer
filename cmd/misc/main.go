...
func main() {
	...
	case "debug-ens":
		err = debugEns()
	case "find-ens-change-block":
		err = findEnsChangeBlock()
	default:
...
func debugEns() error {
	...
	transforms2 = append(transforms2, bt.TransformEnsNameRegistered2)
	...
	if false {
		for i := firstBlock; i < lastBlock; i += batchSize {
			...
			keys2, err := bt.IndexEventsWithTransformersDry(i, i+batchSize, transforms2, 2, cache)
			if err != nil {
				utils.LogError(err, "error indexing from bigtable", 0)
				return err
			}
	...
	for i := int64(19818000); i < 19818000+1000; i++ {
		...
		keys2, err := bt.IndexEventsWithTransformersDry(i, i+100, transforms2, 2, cache)
		if err != nil {
			utils.LogError(err, "error indexing from bigtable", 0)
			return err
		}
...
func findEnsChangeBlock() error {
	...
	if opts.EndBlock == 0 {
		...
		opts.EndBlock = uint64(latestBlockNumber)
	}

	latestBlockRes, err := ResolveAtBlock(opts.Name, uint64(opts.EndBlock))
	if err != nil {
		return err
	}
	...
	if check(startBlockRes, latestBlockRes) {
		return fmt.Errorf("no change between start block %v and latest block %v", opts.StartBlock, opts.EndBlock)
	}

	fmt.Printf("start block: %v (primary: %v), latest block: %v\n", opts.StartBlock, startBlockRes.IsPrimary, opts.EndBlock)

	upperBound := opts.EndBlock
