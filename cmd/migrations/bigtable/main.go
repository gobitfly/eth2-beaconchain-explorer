...
func main() {
	...
	for i := *start; i <= *end; i++ {
		...
		if err != nil {
			utils.LogFatal(err, "deleting epoch error")
		}
		...
		if err != nil {
			utils.LogFatal(err, "getting sync comittee error")
		}
		...
		for i, idxStr := range c.Validators {
			...
			if err != nil {
				utils.LogFatal(err, "parsing validator index to uint error")
			}
		...
		if err != nil {
			utils.LogFatal(err, "getting epoch data error")
		}
		...
		if err != nil {
			utils.LogFatal(err, "wait group error")
		}
	}
