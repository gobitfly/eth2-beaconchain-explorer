...
func FormatAttestationStatus(status uint64) template.HTML {
	if status == 0 {
		return `<span class="badge bg-light text-dark font-weight-normal">Scheduled</span>`
	} else if status == 1 {
		return `<span class="badge bg-success text-white font-weight-normal">Attested</span>`
	} else if status == 2 {
		return `<span class="badge bg-warning text-white font-weight-normal">Missed</span>`
	} else if status == 3 {
		return `<span class="badge bg-warning text-white font-weight-normal">Orphaned</span>`
	} else {
		return "Unknown"
	}
}
...
func FormatAttestationStatusShort(status uint64) template.HTML {
	if status == 0 {
		return `<span title="Scheduled" data-toggle="tooltip" class="badge bg-light text-dark font-weight-normal">Sche.</span>`
	} else if status == 1 {
		return `<span title="Attested" data-toggle="tooltip" class="badge bg-success text-white font-weight-normal">Att.</span>`
	} else if status == 2 {
		return `<span title="Missed" data-toggle="tooltip" class="badge bg-warning text-white font-weight-normal">Miss.</span>`
	} else if status == 3 {
		return `<span title="Orphaned" data-toggle="tooltip" class="badge bg-warning text-white font-weight-normal">Orph.</span>`
	} else if status == 4 {
		return `<span title="Inactivity Leak" data-toggle="tooltip" class="badge bg-danger text-dark font-weight-normal">Leak</span>`
	} else if status == 5 {
		return `<span title="Inactive" data-toggle="tooltip" class="badge bg-light text-dark font-weight-normal">Inac.</span>`
	} else {
		return "Unknown"
	}
}
...
func FormatBlockStatus(status uint64) template.HTML {
	...
	if status == 0 {
		...
		return `<span class="badge bg-success text-white font-weight-normal">Proposed</span>`
	} else if status == 2 {
		return `<span class="badge bg-warning text-white font-weight-normal">Missed</span>`
	} else if status == 3 {
		return `<span class="badge bg-secondary text-white font-weight-normal">Orphaned</span>`
	} else {
		return "Unknown"
	}
}
...
func FormatBlockStatusShort(status uint64) template.HTML {
	...
	if status == 0 {
		...
		return `<span title="Proposed" data-toggle="tooltip" class="badge bg-success text-white font-weight-normal">Prop.</span>`
	} else if status == 2 {
		return `<span title="Missed" data-toggle="tooltip" class="badge bg-warning text-white font-weight-normal">Miss.</span>`
	} else if status == 3 {
		return `<span title="Orphaned" data-toggle="tooltip" class="badge bg-secondary text-white font-weight-normal">Orph.</span>`
	} else {
		return "Unknown"
	}
}
...
func FormatYesNo(yes bool) template.HTML {
	if yes {
		return `<span class="badge bg-success text-white font-weight-normal">Yes</span>`
	}
	return `<span class="badge bg-warning text-white font-weight-normal">No</span>`
}
